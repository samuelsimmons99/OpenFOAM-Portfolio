...
**OpenFOAM v2012 Â· Axisymmetric wedge Â· Compressible reacting flow**
...
|---|------|--------|-------------|
| 1 | **Hot gas** | `rhoCentralFoam` | Combustion products as a single-species perfect gas (M = 22 g/mol, Î³ = 1.2). Density-based explicit solver captures the transonic throat and supersonic plume. |
| 2 | **Premixed combustion** | `rhoReactingFoam` | Câ‚â‚‚Hâ‚‚â‚†/Oâ‚‚ mixture (kerosene surrogate) enters pre-mixed at stoichiometry. Single-step global Arrhenius reaction; JANAF thermodynamics; Sutherland transport. |
| 3 | **Separate injection** | `rhoReactingFoam` | Kerosene (fuel_inlet) and LOX (lox_inlet) are injected separately at O/F â‰ˆ 2.7. Mixing and reaction develop downstream. |

---

## Mesh

All three cases share the same 5-block axisymmetric wedge mesh (5Â° wedge angle) built with `blockMesh`.
...
| Domain: x range | âˆ’0.30 m â†’ 1.50 m |
| Domain: r range | 0 â†’ 0.50 m |
| Total cells | 14 400 |
| Cell distribution | 40 radial Ã— 360 axial (5 blocks) |
...
|----------|-------|-------|
| Molecular weight | 22 g/mol | COâ‚‚/Hâ‚‚O/CO/Hâ‚‚ mixture |
| Î³ | 1.2 | Triatomic-dominated |
| Cp | 2 267 J/(kg K) | From Î³, M |
| Î¼ | 1Ã—10â»â´ Pa s | At 3 000 K |
...
| Patch | p | T | U | Ï |
|-------|---|---|---|---|
| inlet | `totalPressure` 3 MPa | `fixedValue` 3 000 K | `pressureInletOutletVelocity` | `fixedValue` 2.646 kg/mÂ³ |
...
![Mach number contour](nozzle_Mach_contour.png)

Throat Mach = 1.0 confirmed; exit Mach â‰ˆ 3.22, matching the isentropic design value of 3.20 to within 1%.
...
Pre-mixed Câ‚â‚‚Hâ‚‚â‚†/Oâ‚‚ (Y_fuel = 0.2, Y_Oâ‚‚ = 0.8) enters at 500 K and 3 MPa. A single-step global Arrhenius reaction consumes the reactants:

```
2 Câ‚â‚‚Hâ‚‚â‚† + 37 Oâ‚‚ â†’ 24 COâ‚‚ + 26 Hâ‚‚O
A = 5.1Ã—10Â¹Â¹ mÂ³/(kmolÂ·s),  Eâ‚/Ráµ¤ = 15 034 K
```
...
Kerosene and LOX enter through dedicated concentric annular inlets (`fuel_inlet` and `lox_inlet`) at the chamber head. The initial chamber fill is combustion products (COâ‚‚ = 0.693, Hâ‚‚O = 0.307 by mass). Reaction occurs where mixing brings reactants to stoichiometry and above the ignition temperature.

| Parameter | Value |
|-----------|-------|
| Fuel inlet | Câ‚â‚‚Hâ‚‚â‚†, 3 MPa total pressure |
| Oxidiser inlet | Oâ‚‚, 3 MPa total pressure |
| O/F ratio (design) | â‰ˆ 2.7 (stoichiometric for Câ‚â‚‚Hâ‚‚â‚†/Oâ‚‚) |
| Chamber fill (t = 0) | COâ‚‚ + Hâ‚‚O equilibrium products |
...
```
blockMesh  (5-block wedge, 14 400 cells, x = -0.30 â†’ 1.50 m)
     â”‚
     â”œâ”€â”€ rhoCentralFoam      (Case 1: hot gas, density-based explicit)
     â”‚
     â”œâ”€â”€ rhoReactingFoam     (Case 2: premixed Câ‚â‚‚Hâ‚‚â‚†/Oâ‚‚, PIMPLE)
     â”‚
     â””â”€â”€ rhoReactingFoam     (Case 3: separate LOX/kerosene injection)

Post-processing:
  python3 plot_nozzle_cases.py  â†’  mesh, T, p, |U|, species PNGs
```

### Key solver notes

- **`rhoCentralFoam`**: requires consistent `(Ï, p, T)` initial condition - mismatching the outer-boundary density against the interior pressure causes the Newton iteration for temperature to diverge in `hePsiThermo::correct()`. Fix: initialise `Ï` from `p / (R_spec Â· T)` uniformly and use `zeroGradient` for Ï at far-field boundaries.
